...
const ItemOnCart = ({ data }) => {
  console.log("dataaaaa makmukiper");
  ...
  const minItem = () => {
    ...
    setTotalPrice((prevPrice) =>
      prevPrice > 499000 ? prevPrice - 499000 : prevPrice
    );
  };
      ...
      <Div full gap="3" className="flex justify-between items-start ">
        ...
        <Div flex className="justify-between items-start md:gap-5">
          <Div flex column className="text-sm md:text-xl">
            ...
            <p className="text-xs  md:text-lg mt-4 md:mt-10">
              IDR {item.price}
            </p>
  ...
  return (
    <>{isMobileDimension | isTabletDimension ? content : contentOnLaptop}</>
  );
};
